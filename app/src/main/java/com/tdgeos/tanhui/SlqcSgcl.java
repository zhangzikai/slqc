package com.tdgeos.tanhui;

import java.util.ArrayList;
import java.util.List;

import com.tdgeos.dlg.base.MyListDialog;
import com.tdgeos.dlg.base.MyMakeSureDialog;
import com.tdgeos.dlg.yddc.ChooseYm2Dialog;
import com.tdgeos.dlg.yddc.ChooseYmDialog;
import com.tdgeos.dlg.yddc.SgclDialog;
import com.tdgeos.lib.MyFuns;
import com.tdgeos.yangdi.Qianqimgr;
import com.tdgeos.yangdi.Resmgr;
import com.tdgeos.yangdi.Sgcl;
import com.tdgeos.yangdi.YangdiMgr;

import android.app.Activity;
import android.content.Intent;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.os.Bundle;
import android.view.KeyEvent;
import android.view.LayoutInflater;
import android.view.View;
import android.view.Window;
import android.view.WindowManager;
import android.view.animation.Animation;
import android.view.animation.TranslateAnimation;
import android.view.animation.Animation.AnimationListener;
import android.widget.Button;
import android.widget.CheckBox;
import android.widget.CompoundButton;
import android.widget.LinearLayout;
import android.widget.TextView;
import android.widget.Toast;
import android.widget.CompoundButton.OnCheckedChangeListener;

public class SlqcSgcl extends Activity implements View.OnClickListener {
    private int ydh = 0;
    private int iStatus = 0;

    private Button btnClose = null;
    private Button btnQianqi = null;
    private Button btnAdd = null;
    private Button btnDel = null;
    private Button btnSel = null;
    private Button btnSave = null;
    private Button btnFinish = null;

    private LayoutInflater layInflater = null;
    private LinearLayout layList = null;

    private TextView tvSg = null;
    private TextView tvZxg = null;

    private LinearLayout layQq = null;
    private LinearLayout layQqList = null;
    private LinearLayout layQqWu = null;
    private CheckBox cbAll = null;
    private Button btnCopy = null;
    private boolean bVisible = false;
    private List<Sgcl> lstQq = null;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        setContentView(R.layout.slqc_sgcl);
        getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN
                | WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);

        Intent intent = this.getIntent();
        ydh = intent.getIntExtra("ydh", 0);
        int[] ii = YangdiMgr.GetDczt(ydh);
        iStatus = ii[9];

        btnClose = (Button) findViewById(R.id.btn_close);
        btnQianqi = (Button) findViewById(R.id.btn_qq);
        btnSel = (Button) findViewById(R.id.btn_check);
        btnAdd = (Button) findViewById(R.id.btn_add);
        btnDel = (Button) findViewById(R.id.btn_del);
        btnSave = (Button) findViewById(R.id.btn_save);
        btnFinish = (Button) findViewById(R.id.btn_finish);
        btnClose.setOnClickListener(this);
        btnQianqi.setOnClickListener(this);
        btnSel.setOnClickListener(this);
        btnAdd.setOnClickListener(this);
        btnDel.setOnClickListener(this);
        btnSave.setOnClickListener(this);
        btnFinish.setOnClickListener(this);

        layInflater = LayoutInflater.from(this);
        layList = (LinearLayout) findViewById(R.id.lay_list);

        tvSg = (TextView) findViewById(R.id.tv_pjsg);
        tvZxg = (TextView) findViewById(R.id.tv_pjzxg);

        layQq = (LinearLayout) findViewById(R.id.lay_qq);
        layQqList = (LinearLayout) findViewById(R.id.lay_qq_list);
        layQqWu = (LinearLayout) findViewById(R.id.lay_qq_wu);
        cbAll = (CheckBox) findViewById(R.id.cb_all);
        btnCopy = (Button) findViewById(R.id.btn_copy);
        btnCopy.setOnClickListener(this);

        cbAll.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
                int n = layQqList.getChildCount();
                for (int i = 0; i < n; i++) {
                    LinearLayout layRow = (LinearLayout) layQqList.getChildAt(i);
                    CheckBox cb = (CheckBox) layRow.findViewById(R.id.cb_pos);
                    cb.setChecked(arg1);
                }
            }
        });

        initData();
        initQqData();

    }

    @Override
    public void onRestart() {
        super.onRestart();
    }

    @Override
    public void onStart() {
        super.onStart();
    }

    @Override
    public void onResume() {
        super.onResume();
    }

    @Override
    public void onPause() {
        super.onPause();
    }

    @Override
    public void onStop() {
        super.onStop();
    }

    @Override
    public void onDestroy() {
        //更新样地因子中平均树高
        String sg = tvSg.getText().toString();
        String zxg = tvZxg.getText().toString();
        float f_sg = 0;
        float f_zxg = 0;
        try {
            f_sg = Float.parseFloat(sg);
        } catch (Exception e) {
        }
        try {
            f_zxg = Float.parseFloat(zxg);
        } catch (Exception e) {
        }
        if (f_sg > 0.1f) {
            String sql = "update ydyz set pjsg = '" + f_sg + "' where ydh = '" + ydh + "'";
            YangdiMgr.ExecSQL(ydh, sql);
        } else if (f_zxg > 0.1f) {
            String sql = "update ydyz set pjsg = '" + f_zxg + "' where ydh = '" + ydh + "'";
            YangdiMgr.ExecSQL(ydh, sql);
        }

        updateShugao();

        super.onDestroy();
    }

    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            return super.onKeyDown(keyCode, event);
        } else {
            return super.onKeyDown(keyCode, event);
        }
    }

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                case 101: {
                    break;
                }
                default:
                    break;
            }
        }
    }

    @Override
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_close: {
                this.finish();
                break;
            }
            case R.id.btn_qq: {
                if (bVisible) {
                    TranslateAnimation ta = new TranslateAnimation(0, 0, 0, layQq.getHeight());
                    ta.setDuration(200);
                    ta.setFillAfter(true);
                    ta.setAnimationListener(new AnimationListener() {
                        @Override
                        public void onAnimationStart(Animation arg0) {
                        }

                        @Override
                        public void onAnimationRepeat(Animation arg0) {
                        }

                        @Override
                        public void onAnimationEnd(Animation arg0) {
                            layQq.setVisibility(8);
                        }
                    });
                    layQq.startAnimation(ta);
                    bVisible = false;
                } else {
                    TranslateAnimation ta = new TranslateAnimation(0, 0, layQq.getHeight(), 0);
                    ta.setDuration(200);
                    ta.setFillAfter(true);
                    ta.setAnimationListener(new AnimationListener() {
                        @Override
                        public void onAnimationStart(Animation arg0) {
                            layQq.setVisibility(1);
                        }

                        @Override
                        public void onAnimationRepeat(Animation arg0) {
                        }

                        @Override
                        public void onAnimationEnd(Animation arg0) {
                        }
                    });
                    layQq.startAnimation(ta);
                    bVisible = true;
                }
                break;
            }
            case R.id.btn_copy: {
                List<String> items = new ArrayList<String>();
                items.add("覆盖已录入数据");
                items.add("跳过已录入数据");
                MyListDialog dlg = new MyListDialog(this, "选项", items);
                int r = dlg.showDialog();
                if (r == -1) return;

                int n = layQqList.getChildCount();
                for (int i = 0; i < n; i++) {
                    LinearLayout layRow = (LinearLayout) layQqList.getChildAt(i);
                    CheckBox cb = (CheckBox) layRow.findViewById(R.id.cb_pos);
                    if (cb.isChecked()) {
                        Sgcl item = lstQq.get(i);
                        item.sz = Resmgr.GetSzNewCodeByOldCode(item.sz);
                        boolean b = YangdiMgr.IsHaveSgcl(ydh, item);
                        if (b) {
                            if (r == 0) {
                                String sql = "update sgcl set "
                                        + "sz = '" + item.sz + "', "
                                        + "xj = '" + item.xj + "', "
                                        + "sg = '" + item.sg + "', "
                                        + "zxg = '" + item.zxg + "' "
                                        + " where ydh = '" + ydh + "' and ymh = '" + item.ymh + "'";
                                YangdiMgr.ExecSQL(ydh, sql);
                            } else {
                                continue;
                            }
                        } else {
                            YangdiMgr.AddSgcl(ydh, item);
                        }
                    }
                }
                initData();
                cbAll.setChecked(false);
                break;
            }
            case R.id.btn_save: {
                List<String> lstError = new ArrayList<String>();
                List<String> lstWarn = new ArrayList<String>();
                YangdiMgr.CheckSgcl(ydh, lstError, lstWarn);
                if (lstError.size() > 0) {
                    iStatus = 1;
                }
                if (iStatus != 2) {
                    iStatus = 1;
                }
                String sql = "update dczt set sgcl = '" + iStatus + "' where ydh = '" + ydh + "'";
                YangdiMgr.ExecSQL(ydh, sql);
                break;
            }
            case R.id.btn_finish: {
                List<String> lstError = new ArrayList<String>();
                List<String> lstWarn = new ArrayList<String>();
                YangdiMgr.CheckSgcl(ydh, lstError, lstWarn);
                if (lstError.size() > 0) {
                    iStatus = 1;
                } else {
                    iStatus = 2;
                }
                String sql = "update dczt set sgcl = '" + iStatus + "' where ydh = '" + ydh + "'";
                YangdiMgr.ExecSQL(ydh, sql);
                finish();
                break;
            }
            case R.id.btn_check: {
                int n = YangdiMgr.GetYangmuCount(ydh);
                if (n <= 0) {
                    Toast.makeText(this, "尚未指定优势树种！", 1).show();
                    break;
                }
                ChooseYm2Dialog dlg = new ChooseYm2Dialog(this, ydh);
                List<Sgcl> lst = dlg.showDialog();
                if (lst != null) {
                    for (int i = 0; i < lst.size(); i++) {
                        YangdiMgr.AddSgcl(ydh, lst.get(i));
                    }
                    initData();
                }
                break;
            }
            case R.id.btn_add: {
                //if(layList.getChildCount() >= 3)
                //{
                //	Toast.makeText(this, "只需调查3株平均木！", 1).show();
                //	break;
                //}
                SgclDialog dlg = new SgclDialog(this, null, ydh);
                Sgcl item = dlg.showDialog();
                if (item != null) {
                    YangdiMgr.AddSgcl(ydh, item);
                    initData();
                }
                break;
            }
            case R.id.btn_del: {
                MyMakeSureDialog dlg = new MyMakeSureDialog(this, "删除", "删除后将不可恢复，是否继续删除？", "删除", "取消");
                if (dlg.showDialog()) {
                    delRow();
                }
                break;
            }
            default: {
                LinearLayout layRow = null;
                try {
                    layRow = (LinearLayout) v;
                } catch (Exception e) {
                }
                if (layRow != null) {
                    int xh = (Integer) layRow.getTag();
                    Sgcl sgcl = YangdiMgr.GetSgcl(ydh, xh);
                    SgclDialog dlg = new SgclDialog(this, sgcl, ydh);
                    Sgcl item = dlg.showDialog();
                    if (item != null) {
                        YangdiMgr.UpdateSgcl(ydh, item);
                        TextView tvYmh = (TextView) layRow.findViewById(R.id.tv_ymh);
                        TextView tvSz = (TextView) layRow.findViewById(R.id.tv_sz);
                        TextView tvXj = (TextView) layRow.findViewById(R.id.tv_xj);
                        TextView tvSg = (TextView) layRow.findViewById(R.id.tv_sg);
                        TextView tvZxg = (TextView) layRow.findViewById(R.id.tv_zxg);
                        tvYmh.setText(String.valueOf(item.ymh));
                        tvSz.setText(Resmgr.GetSzName(item.sz));
                        tvXj.setText(String.valueOf(item.xj));
                        if (item.sg > 0) tvSg.setText(String.valueOf(item.sg));
                        if (item.zxg > 0) tvZxg.setText(String.valueOf(item.zxg));
                        pj();
                    }
                }
                break;
            }
        }
    }

    private void initData() {
        layList.removeAllViews();
        List<Sgcl> lst = YangdiMgr.GetSgclList(ydh);
        if (lst != null && lst.size() > 0) {
            for (int i = 0; i < lst.size(); i++) {
                addRow(lst.get(i));
            }
        }
        pj();
    }

    private void addRow(Sgcl item) {
        if (item == null) return;
        LinearLayout layRow = (LinearLayout) layInflater.inflate(R.layout.slqc_sgcl_item, null);
        layRow.setTag(item.xh);
        layRow.setOnClickListener(this);
        CheckBox cb = (CheckBox) layRow.findViewById(R.id.cb_pos);
        cb.setTag(item.xh);
        TextView tvYmh = (TextView) layRow.findViewById(R.id.tv_ymh);
        TextView tvSz = (TextView) layRow.findViewById(R.id.tv_sz);
        TextView tvXj = (TextView) layRow.findViewById(R.id.tv_xj);
        TextView tvSg = (TextView) layRow.findViewById(R.id.tv_sg);
        TextView tvZxg = (TextView) layRow.findViewById(R.id.tv_zxg);
        tvYmh.setText(String.valueOf(item.ymh));
        tvSz.setText(Resmgr.GetSzName(item.sz));
        tvXj.setText(String.valueOf(item.xj));
        if (item.sg > 0) tvSg.setText(String.valueOf(item.sg));
        if (item.zxg > 0) tvZxg.setText(String.valueOf(item.zxg));
        layList.addView(layRow);
    }

    private void delRow() {
        int n = layList.getChildCount();
        for (int i = n - 1; i >= 0; i--) {
            LinearLayout layRow = (LinearLayout) layList.getChildAt(i);
            CheckBox cb = (CheckBox) layRow.findViewById(R.id.cb_pos);
            if (cb.isChecked()) {
                int xh = (Integer) cb.getTag();
                layList.removeViewAt(i);
                YangdiMgr.DelSgcl(ydh, xh);
            }
        }
        pj();
    }

    private void pj() {
        float sg = 0;
        float zxg = 0;
        List<Sgcl> lst = YangdiMgr.GetSgclList(ydh);
        if (lst != null && lst.size() > 0) {
            for (int i = 0; i < lst.size(); i++) {
                if (lst.get(i).sg > 0) sg += lst.get(i).sg;
                if (lst.get(i).zxg > 0) zxg += lst.get(i).zxg;
            }
            sg /= lst.size();
            zxg /= lst.size();
            sg = MyFuns.MyDecimal(sg, 1);
            zxg = MyFuns.MyDecimal(zxg, 1);
        }
        if (sg > 0) tvSg.setText(String.valueOf(sg));
        if (zxg > 0) tvZxg.setText(String.valueOf(zxg));
    }

    private void initQqData() {
        layQqList.removeAllViews();
        lstQq = Qianqimgr.GetQqSgclList(ydh);
        if (lstQq != null && lstQq.size() > 0) {
            for (int i = 0; i < lstQq.size(); i++) {
                addQqRow(lstQq.get(i));
            }
        } else {
            layQqWu.setVisibility(1);
        }
    }

    private void addQqRow(Sgcl item) {
        if (item == null) return;
        LinearLayout layRow = (LinearLayout) layInflater.inflate(R.layout.slqc_sgcl_item, null);
        TextView tvYmh = (TextView) layRow.findViewById(R.id.tv_ymh);
        TextView tvSz = (TextView) layRow.findViewById(R.id.tv_sz);
        TextView tvXj = (TextView) layRow.findViewById(R.id.tv_xj);
        TextView tvSg = (TextView) layRow.findViewById(R.id.tv_sg);
        TextView tvZxg = (TextView) layRow.findViewById(R.id.tv_zxg);
        tvYmh.setText(String.valueOf(item.ymh));

        int code = item.sz;
        String name = Resmgr.GetSzName(code);
        String sz = "";
        if (code > 0 && !name.equals("")) {
            sz = code + " " + name;
        } else {
            sz = "";
        }
        tvSz.setText(sz);
        tvXj.setText(String.valueOf(item.xj));
        if (item.sg > 0) tvSg.setText(String.valueOf(item.sg));
        if (item.zxg > 0) tvZxg.setText(String.valueOf(item.zxg));
        layQqList.addView(layRow);
    }

    private void updateShugao() {
        List<Integer> lstSzs = new ArrayList<Integer>();
        String sql = "select szdm from mmjc where ydh = '" + ydh + "' "
                //+ "and kjdlxh not in(1,2,3) "
                + "and (jclx not in(13,14,15,17,24)) "
                + "and bqxj >= '5' "
                + "group by szdm";
        String dbFile = YangdiMgr.getDbFile(ydh);
        SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(dbFile, null);
        Cursor cursor = null;
        cursor = db.rawQuery(sql, null);
        if (cursor != null) {
            int n = cursor.getCount();
            int m = cursor.getColumnCount();
            if (n == 0 || m == 0) {
                cursor.close();
                db.close();
            } else {
                cursor.moveToFirst();
                for (int i = 0; i < n; i++) {
                    int v = cursor.getInt(0);
                    lstSzs.add(v);
                    cursor.moveToNext();
                }
                cursor.close();
                db.close();
            }
        } else {
            db.close();
        }

        sql = "delete from sgcl_jtsz";
        YangdiMgr.ExecSQL(ydh, sql);

        List<Sgcl> sgcls = YangdiMgr.GetSgclList(ydh);
        int count = sgcls.size();

        for (int i = 0; i < lstSzs.size(); i++) {
            int n = 0;
            int sz = lstSzs.get(i);
            float sg = 0;
            for (int j = 0; j < count; j++) {
                if (sgcls.get(j).sz == sz) {
                    n++;
                    sg += sgcls.get(j).sg;
                }
            }
            if (n > 0) sg /= n;

            sql = "insert into sgcl_jtsz(ydh,sz,gd) values('" + ydh + "', '" + lstSzs.get(i) + "', '" + sg + "')";
            YangdiMgr.ExecSQL(ydh, sql);
        }


    }
}
